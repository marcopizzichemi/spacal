  ...
  material(mat)
{
  ...
  NfibresAlongY = floor( (module_yx-2*margin-0.5*fibre_distance)/fibre_distance ) + 1 ;
  
  start_x = 0.5 * ( module_xy - floor ((module_xy - 2 * margin)/(fibre_distance * 0.8660)) * (fibre_distance * 0.8660) );
  start_y = 0.5 * ( module_yx - fibre_distance*(NfibresAlongY-0.5) );
}
...
void FibreParameterisation::ComputeTransformation(const G4int copyNo, G4VPhysicalVolume* physVol) const
{
  ...
  G4double y_c = -0.5*module_yx+start_y + count_y*fibre_distance        + 0.5*fibre_distance*(count_x%2);

  //___________________________________________________
  // G4int count_xs = floor(copyNo/NfibresAlongY);
  // G4int count_ys = int(copyNo)%int(NfibresAlongY);
  
  // G4double x_cs = -0.5*module_xy+start_x + count_x*fibre_distance*0.8660;
  // G4double y_cs = -0.5*module_xy+start_y + count_y*fibre_distance        + 0.5*fibre_distance*(count_x%2);
  ...
  //  G4ThreeVector origin(x_cs,y_cs,z_cs);
  // physVol -> SetTranslation(origin);
  // physVol -> SetRotation(0);
  ...
  material(mat)
{
  ...
  start_y = 0.5 * ( module_yx - hole_distance*(NholesAlongY-0.5) );
}
